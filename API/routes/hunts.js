...
router.get("/", ensureToken, async function (req, res, next) {
  const searchTerm = req.query.search ? {$text: {$search: req.query.search}} : {};
  const found = await Hunt.find(searchTerm, {author:1, title:1, description:1, ratings:1, downloads:1});
